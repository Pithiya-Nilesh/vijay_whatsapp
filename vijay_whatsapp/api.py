    ...
    if doc.contact_mobile and doc.custom_send_whatsapp_message:
        if check_whatsapp_api():
            ...
            if doc.sales_partner:
                sp_doc_name = frappe.db.sql(
                    " select parent from `tabDynamic Link` where link_name=%s ",
                    (doc.sales_partner),
                    as_dict=True,
                )
                if sp_doc_name:
                    sp_number = frappe.db.sql(
                        " select phone from `tabContact` where name=%s ",
                        (sp_doc_name[0]["parent"]),
                        as_dict=True,
                    )
                    if sp_number:
            ...
            message_template = """Dear Sir,\n
                                ...
                                For any query, please call 9925273456 / 9825212056.\n
                                \n
                                From:\n
    ...
    if doc.contact_mobile and doc.custom_send_whatsapp_message:
        if check_whatsapp_api():
            ...
            message_template = """Dear Sir,
                                ...
                                For any Query Call 9925273456 / 9825212056
    ...
    if check_scheduler_is_enable():
        if check_whatsapp_api():
            data = frappe.db.get_list(
                ...
                filters={
                    "due_date": "2023-10-29",
                    "custom_send_whatsapp_message": 1,
            ...
            for name in data:
                ...
                if doc.contact_mobile:
                    ...
                    # send_whatsapp_message(whatsapp_no, 'Your+Sales+Order+is+Created.', frappe.utils.get_url()+file["file_url"], file["file_name"])
                    enqueue(
                        "vijay_whatsapp.api.send_whatsapp_message",
                        numbers=whatsapp_no,
                        message="Your+Sales+Invoice+is+Outstanding.",
                        file_url=file_url,
    ...
    if doc.custom_send_whatsapp_message:
        if check_whatsapp_api():
            ...
            if doc.payment_type == "Internal Transfer":
                message = (
                    f"Internal Payment Entry From {doc.paid_from} To {doc.paid_to}."
                )
    ...
    if doc.mobile_no:
        if check_whatsapp_api():
            ...
            enqueue(
                "vijay_whatsapp.api.send_whatsapp_message",
                numbers=whatsapp_no,
                message="Your OutStanding Invoice Details",
                file_url=file_url,
                filename=file["file_name"],
                docname=doc.name,
                doctype=doc.doctype,
...
def send_whatsapp_message(numbers, message, file_url, filename, docname, doctype):
    ...
    for number in numbers:
        ...
        if "status" in data and data["status"] == "success":
            enqueue(
                "vijay_whatsapp.api.set_comment",
                doctype=doctype,
                docname=docname,
                owner=owner,
                content="<div class='card'><b style='color: green' class='px-2 pt-2'>Whatsapp Message Sent: </b> <span class='px-2 pb-2'>Your whatsapp message send successfully.</span></div>",
            )
            enqueue(
                "vijay_whatsapp.api.set_whatsapp_log",
                doctype=doctype,
                docname=docname,
                whatsapp_no=number,
            ...
            enqueue(
                ...
                docname=docname,
                owner=owner,
                content=f"<div class='card'><b style='color: red' class='px-2 pt-2'>Whatsapp Message Not Sent: </b> <span class='px-2 pb-2'>{response.text}</span></div>",
            )
            enqueue(
                "vijay_whatsapp.api.set_whatsapp_log",
                doctype=doctype,
...
def get_whatsapp_credentials():
    ...
    instance_id = get_decrypted_password(
        "Whatsapp Settings", "Whatsapp Settings", "instance_id", raise_exception=False
    )
    access_token = get_decrypted_password(
        "Whatsapp Settings", "Whatsapp Settings", "access_token", raise_exception=False
    )
...
def check_whatsapp_api():
    ...
    if enable == 0:
        frappe.msgprint(
            "Please Enable WhatsApp Api in Whatsapp Settings To Send Whatsapp Message."
        )
        return False
    else:
        return True
...
def create_and_store_file(doc, whatsapp_no, message):
    ...
    if doc.doctype == "Sales Order":
        html_content = frappe.get_print(
            doc.doctype, doc.name, "Sales Order Print Designer"
        )
    ...
    sent_file = frappe.get_doc(
        {"doctype": "Sent File", "file_path": file["file_url"], "sent_date": today()}
    )
...
def delete_sent_file():
    ...
    doc_name = frappe.db.get_list(
        "Sent File",
        filters=[["sent_date", "<", before_14_days]],
...
def get_mobile_numbers(roles):
    ...
    users = frappe.get_all(
        "User", filters={"role": ["in", roles]}, fields=["first_name", "mobile_no"]
    )
...
def send_report_pdf(name, report_name):
    from vijay_whatsapp.report import (
        get_receivable_report_content,
        get_general_report_content,
    )
...
def check(numbers="", message="", file_url=""):
    ...
    data = {
        "message": {
            "key": {
                "remoteJid": "917990915950@c.us",
                "fromMe": True,
                "id": "BAE51B3047BA6F4C",
            },
            "message": {
                "documentMessage": {
                    "url": "https://mmg.whatsapp.net/v/t62.7119-24/40116021_1273910713269040_1288344065129231328_n.enc?ccb=11-4&oh=01_AdS2Te3IYQyScs-n1wO7_7v1EVak0e0Mp-Lh9UYPlIeypg&oe=65470DFA&_nc_sid=000000&mms3=true",
                    "mimetype": "application/pdf",
                    "fileSha256": "FHjwuF3x9HAqsbTACcfze2CgXvbxceiNZOOta/7Z4Kg=",
                    "fileLength": "24838",
                    "mediaKey": "6lOdvTjXuwTCLYNKdWm9YMTozmD/HN4x+6rJ9gQqBJo=",
                    "fileName": "Payment Entry-ACC-PAY-2023-00002.pdf",
                    "fileEncSha256": "yoTZI27Oi5x81EmIVymkAbuqaJlBQcGZmy86bysvy6E=",
                    "directPath": "/v/t62.7119-24/40116021_1273910713269040_1288344065129231328_n.enc?ccb=11-4&oh=01_AdS2Te3IYQyScs-n1wO7_7v1EVak0e0Mp-Lh9UYPlIeypg&oe=65470DFA&_nc_sid=000000",
                    "mediaKeyTimestamp": "1696569769",
                    "caption": "asdf",
                }
            },
            "messageTimestamp": "1696569769",
        }
    }
